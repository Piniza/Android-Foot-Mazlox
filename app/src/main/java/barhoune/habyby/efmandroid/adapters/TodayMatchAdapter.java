...
import barhoune.habyby.efmandroid.TodayMatchesFragmentDirections;
import barhoune.habyby.efmandroid.models.match.Match;

public class TodayMatchAdapter extends RecyclerView.Adapter<TodayMatchAdapter.ViewHolder> {
    ...
    @NonNull
    @Override
    public TodayMatchAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_match, parent, false);
        return new TodayMatchAdapter.ViewHolder(view);
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Match match = mMatches.get(position);
            ...
            Picasso.get().load(match.getMatchAwayTeam().getAwayTeamCrest()).error(R.drawable.awaycrest).resize(100,100).into(holder.imgAway);
            Picasso.get().load(match.getMatchHomeTeam().getHomeTeamCrest()).error(R.drawable.homecrest).resize(100,100).into(holder.imgHome);
          ...
          holder.itemView.setOnClickListener(view -> {
              TodayMatchesFragmentDirections.ActionTodayMatchesFragmentToMatchDetails action;
              action= TodayMatchesFragmentDirections.actionTodayMatchesFragmentToMatchDetails(match.getMatchId());
